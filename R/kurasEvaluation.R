...
beispielAuswertung.1 <- function # Beispiel-Auswertung 1
...
# kuras_auswertung -------------------------------------------------------------
kuras_auswertung <- function # kuras_auswertung
### kuras_auswertung
(
...
{ 
  ...
  cat(paste(1:anzahlEffekte, effekte$eName, sep = ". ", collapse = "\n"))
  cat("\n")
  ...
  setMargins(left = 25, top=2, bottom=2)
...
anzahlIndikatorenProEffekt <- function # Anzahl an Indikatoren pro Effekt
...
)
{
  ...
  ### data frame mit Spalten \emph{eID} (Effekt-ID), \emph{eName} (Effekt-Name),
  ### \emph{x} (Anzahl Indikatoren fuer den Effekt).
}

# anzahlWerteProIndikator ------------------------------------------------------
anzahlWerteProIndikator <- function # Anzahl an Werten pro Indikator
...
)
{
  ...
  ### data frame mit Spalten \emph{iID} (Indikator-ID), \emph{iEffektID} 
