...
#' readKurasSamplerFileByName
#' 
#' @param bottlesToConsider arguments passed to \code{\link{generateSampleTimesFromFlaschenbericht}}
#' 
readKurasSamplerFileByName <- function # readKurasSamplerFileByName
### readKurasSamplerFileByName
(
  samplerFile, 
  bottlesToConsider = NA
  ### arguments passed to \code{\link{generateSampleTimesFromFlaschenbericht}}
)
{
...
#' 
readKurasSamplerFileByName.R <- function # readKurasSamplerFileByName.R
### read file generated by KURAS_convertAutosamplerFiles_RBFAB.R from 
### "_flaschenbericht" file
(
  samplerFile
...
#'   "Flaschenbericht"
#' 
#' @param content data frame with columns \emph{timeFrom}, \emph{timeTo}, \emph{pnSoll},
#'   \emph{pnIst},
#' 
generateSampleTimesFromFlaschenbericht <- function # generateSampleTimesFromFlaschenbericht
### generate equidistant sample times from the informatin given in the
### "Flaschenbericht"
(
  content,
  ### data frame with columns \emph{timeFrom}, \emph{timeTo}, \emph{pnSoll},
  ### \emph{pnIst},
...
)
{
  ...
  for (i in seq_len(nrow(content))) {
    ...
    singleSamples <- data.frame(
      ...
      sampleTime = content$timeFrom[i] + (0:(n-1))*timestep + timestep/2,
      ...
      result = ifelse(seq_len(n) <= content$pnIst[i], text.success, text.failure),
